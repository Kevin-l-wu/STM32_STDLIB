...
uint16_t* gAdcConvertResult[CHANNEL_NUM] = {0}; 

extern void MainDelay(unsigned int n);


void BspAdcCont(void)
{ 	
...
void BspGetAdcValCont(u16* pResult)   
{
	...
	if(pResult != NULL)
	{
		for(index = 0; index < CHANNEL_NUM; index++)
		{
			...
			*(pResult + index) = ADC_GetConversionValue(ADC1);	//返回最近一次ADC1规则组的转换结果
		}
	}
}
