...
//	printf("SDCardInfo.SD_csd = 0x%x\n", SDCardInfo.SD_csd);
//	printf("SDCardInfo.SD_cid = 0x%x\n", SDCardInfo.SD_cid);
	printf("SDCardInfo.CardCapacity = 0x%llx\n", SDCardInfo.CardCapacity);
	printf("SDCardInfo.CardBlockSize = 0x%x\n", SDCardInfo.CardBlockSize);
  }
...
SDTransferState SD_GetStatus(void)
{
 ...
 // printf("SD_GetStatus(): cardstate = 0x%x\n", cardstate);
...
//  printf("StopCondition = 0x%x\n", StopCondition);
//  printf("TransferEnd = 0x%x\n", TransferEnd);
//  printf("SD_ProcessIRQSrc end----------------------\n");
  return(TransferError);
}
...
static SD_Error SDEnWideBus(FunctionalState NewState)
{
  ...
  printf("SDEnWideBus()1: errorstatus = 0x%x\n", errorstatus);
  ...
  printf("SDEnWideBus()2: NewState = 0x%x\n", NewState);
  printf("SDEnWideBus()3: scr[1] = 0x%x\n", scr[1]);
  ...
  if (NewState == ENABLE)
  {
    ...
    if ((scr[1] & SD_WIDE_BUS_SUPPORT) != SD_ALLZERO)
    {
      ...
      errorstatus = CmdResp1Error(SD_CMD_APP_CMD);
	  printf("SDEnWideBus()4: errorstatus = 0x%x\n", errorstatus);
...
static SD_Error FindSCR(uint16_t rca, uint32_t *pscr)
{
  ...
  printf("FindSCR()1: errorstatus = 0x%x\n", errorstatus);
  ...
  printf("FindSCR()2: SDIO->STA = 0x%x\n", SDIO->STA);
  printf("FindSCR()2: SDIO->FIFOCNT = 0x%x\n", SDIO->FIFOCNT);

 // while (!(SDIO->STA & (SDIO_FLAG_RXOVERR | SDIO_FLAG_DCRCFAIL | SDIO_FLAG_DTIMEOUT | SDIO_FLAG_DBCKEND | SDIO_FLAG_STBITERR)))
  while (!(SDIO->STA & (SDIO_FLAG_RXOVERR | SDIO_FLAG_DCRCFAIL | SDIO_FLAG_DTIMEOUT | SDIO_FLAG_STBITERR)))
  {
    if (SDIO_GetFlagStatus(SDIO_FLAG_RXDAVL) != RESET)
    {
      printf("FindSCR()2: index = 0x%x\n", index);
	  printf("FindSCR()2: SDIO->STA = 0x%x\n", SDIO->STA);
	  *(tempscr + index) = SDIO_ReadData();
      index++;
	  printf("FindSCR()2: tempscr[0] = 0x%x\n", tempscr[0]);
      printf("FindSCR()2: tempscr[1] = 0x%x\n", tempscr[1]);
  ...
  printf("FindSCR()2: errorstatus = 0x%x\n", errorstatus);
  ...
  printf("FindSCR()3: errorstatus = 0x%x\n", errorstatus);
